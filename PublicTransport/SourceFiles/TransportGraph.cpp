...
unsigned TransportGraph::findShortestTimeBetweenBusStops(std::string& startBusStop, std::string& endBusStop, unsigned startTime, std::vector<vertexAndDistancePair>& path)
{
	return dijkstraAlgorithmForPublicTransport(
		...
		{
			return this->calculateDynEdgeWeightShortestTime(startTime, currTime, currBus, nextBus, currStop, nextStop);
		},
		startBusStop, endBusStop, startTime, path
	);
}

unsigned TransportGraph::findMinBusesTakenPath(std::string& startBusStop, std::string& endBusStop, std::vector<vertexAndDistancePair>& path)
{
	...
	return dijkstraAlgorithmForPublicTransport(
		...
		{
			return this->calculateDynEdgeWeightMinBusesTaken(currBus, nextBus);
		},
		startBusStop, endBusStop, 0, path
	);
}

unsigned TransportGraph::findMinTimeSpentOutsideVehiclePath(unsigned startTime, std::string& startBusStop, std::string& endBusStop, std::vector<vertexAndDistancePair>& path)
{
	return dijkstraAlgorithmForPublicTransport(
		...
		{
			return this->calculateDynEdgeWeightMinTimeSpent(currTime, currBus, nextBus, currStop, nextStop);
		},
		startBusStop, endBusStop, startTime, path
	);
}
...
bool TransportGraph::checkBusStopExistence(const std::string& busStopName)
{
	if (nameToBusStop.find(busStopName) == nameToBusStop.end())
	{
		return false;
...
{
	const auto& currBusStopPtr = nameToBusStop.find(busStopName);
	if (currBusStopPtr == nameToBusStop.end())
	{
		std::cerr << "Bus Stop with such name doesn't exist!";
		return;
	}
	std::cout << "Passing buses through " + busStopName + ":\n";
	for (const auto& pair : currBusStopPtr->second.getBusToSchedule())
	{
		std::cout << pair.first << " ";
	}
	return;
...
std::pair<unsigned, unsigned> TransportGraph::calculateDynEdgeWeightShortestTime(unsigned startTime, unsigned currTimeFromStart, unsigned currBusTakenNumber, unsigned nextBusNumber, const std::string& currBusStopName, const std::string& nextBusStopName)
{
...
std::pair<unsigned,unsigned> TransportGraph::calculateDynEdgeWeightMinBusesTaken(unsigned currBusTakenNumber, unsigned nextBusNumber)
{
	if (currBusTakenNumber == nextBusNumber)
	{
		return { 0, 0 };
...
std::pair<unsigned, unsigned> TransportGraph::calculateDynEdgeWeightMinTimeSpent(unsigned currTime, unsigned currBusTakenNumber, unsigned nextBusNumber, const std::string& currBusStopName, const std::string& nextBusStopName)
{
	const auto& startBusStop = publicTransportGrid.find(currBusStopName);
	const auto& nextBusStop = publicTransportGrid.find(nextBusStopName);
	if (currBusTakenNumber == nextBusNumber)
